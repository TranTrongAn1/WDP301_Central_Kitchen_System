...
import {
  logisticsOrderStatusBadgeClass,
  logisticsOrderStatusVi,
  tripStatusBadgeClass,
  tripStatusVi,
} from '@/shared/lib/statusLabels';
import { useThemeStore } from '@/shared/zustand/themeStore';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/features/manager/components/ui/Select';
...
const AdminDashboard = () => {
  ...
  // Filter states — dùng filterKey để trigger re-fetch thay vì phụ thuộc selectedYear/Quarter/Month trực tiếp
  const [selectedYear, setSelectedYear] = useState<string>(String(getCurrentYear()));
  const [selectedQuarter, setSelectedQuarter] = useState<string>('all');
  const [selectedMonth, setSelectedMonth] = useState<string>('all');
  const [filterKey, setFilterKey] = useState(0); // tăng khi filter thay đổi
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        setLoading(true);
        const today = new Date().toISOString().slice(0, 10);
        const [orderRes, aggregateRes] = await Promise.all([
          OrderApi.getListWithCount().catch(() => ({ count: 0, data: [] })),
          OrderApi.getAggregate(today).catch(() => ({ data: [] })),
        ]);
        ...
        // Calculate orders by month from ALL orders, then filter by selected year
        const monthMap: Record<string, { orders: number; revenue: number }> = {};
        for (let m = 1; m <= 12; m++) {
          monthMap[m] = { orders: 0, revenue: 0 };
        }
  ...
  }, [filterKey]); // filterKey tăng khi user thay đổi bộ lọc
  ...
  const statCards = [
    ...
    { label: 'Kế hoạch sản xuất', value: loading ? 0 : filteredPlans.length, icon: ChefHat, gradient: 'from-amber-500 to-orange-600', href: '/admin/production' },
    ...
    { label: 'Đơn hàng', value: loading ? 0 : filteredOrders.length, icon: ClipboardList, gradient: 'from-blue-500 to-indigo-600', href: '/admin/orders' },
    { label: 'Chuyến giao', value: loading ? 0 : filteredTrips.length, icon: Truck, gradient: 'from-cyan-500 to-blue-600', href: '/admin/shipments' },
    {
      label: 'Tồn kho',
                        ...
                        <td className="py-1.5 pr-2">
                          <span
                            className={cn(
                              ...
                              logisticsOrderStatusBadgeClass(String(o.status))
                            )}
                          >
                            {logisticsOrderStatusVi(String(o.status))}
                          </span>
                        </td>
