...
const Shipments = () => {
    ...
    return (
        <div className="animate-in fade-in duration-500 pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {currentTrips.length === 0 ? (
                    <div className="col-span-full py-24 rounded-2xl border-2 border-dashed border-border bg-card/50 flex flex-col items-center justify-center">
                        ...
                        <p className="font-bold uppercase text-sm tracking-widest text-muted-foreground">Chưa có chuyến hàng nào</p>
                    </div>
                ) : (
                    currentTrips.map((trip) => (
                        <div key={trip._id} className="rounded-xl border border-border bg-card p-5 flex flex-col transition-all group relative overflow-hidden hover:shadow-md hover:border-primary/20">
                            ...
                            <div className="space-y-1 mb-5 flex-1">
                                ...
                                {(trip as any).vehicleType && (
                                    <p className="text-[10px] font-bold text-primary uppercase">
                                        Xe: {(trip as any).vehicleType.name}
                                    </p>
                                )}
                            </div>

                            <div className="pt-4 border-t border-border flex justify-between items-center text-xs font-medium uppercase text-muted-foreground">
                                <span>Khởi tạo:</span>
                                <span className="font-semibold text-card-foreground">{new Date(trip.createdAt).toLocaleDateString("vi-VN")}</span>
                            </div>
                ...
                <div className="flex justify-end items-center gap-2 mt-8 select-none">
                    ...
                    <div className="flex items-center gap-1.5">
                        {getPageNumbers().map((page, index) => (
                            <button key={index} onClick={() => typeof page === 'number' && handlePageChange(page)} className={cn("min-w-[36px] h-9 px-3 rounded-lg text-sm font-bold", currentPage === page ? "bg-primary text-primary-foreground shadow-md" : "bg-secondary text-muted-foreground hover:bg-primary/10")}>
                                {page}
                            </button>
                        ))}
                    </div>
                    ...
                    <div className={`w-full max-w-2xl rounded-[32px] border shadow-2xl overflow-hidden animate-in zoom-in-95 flex flex-col ${darkMode ? 'bg-[#25252A] border-gray-700' : 'bg-white border-gray-200'}`}>
                        ...
                        <div className="p-6 overflow-y-auto space-y-4 custom-scrollbar max-h-[60vh]">
                            ...
                            <div className="space-y-2">
                                <p className="text-[10px] font-bold uppercase text-muted-foreground">Danh sách đơn hàng khả dụng</p>
                                {availableOrders.length === 0 ? (
                                    <p className="text-center py-4 text-xs font-bold text-muted-foreground italic">Không có đơn hàng nào chờ vận chuyển.</p>
                                ) : (
                                    availableOrders.map((order) => (
                                        <label key={order._id} className={cn("flex items-center justify-between p-4 rounded-xl border-2 transition-all cursor-pointer", selectedOrderIds.includes(order._id) ? 'border-orange-500 bg-orange-500/10' : 'border-border bg-card hover:border-orange-300')}>
                                            <div className="flex items-center gap-4">
                                                <input type="checkbox" className="hidden" checked={selectedOrderIds.includes(order._id)} onChange={() => setSelectedOrderIds(prev => prev.includes(order._id) ? prev.filter(id => id !== order._id) : [...prev, order._id])} />
                                                <div className={cn("w-6 h-6 rounded-md border-2 flex items-center justify-center transition-all", selectedOrderIds.includes(order._id) ? 'bg-orange-500 border-orange-500' : 'border-gray-300')}>
                                                    {selectedOrderIds.includes(order._id) && <CheckCircle className="w-4 h-4 text-white" />}
                                                </div>
                                                <div>
                                                    <p className="font-black text-sm uppercase">{order.orderCode || order._id.slice(-6)}</p>
                                                    <p className="text-[10px] font-bold text-muted-foreground uppercase">Ngày giao: {new Date(order.requestedDeliveryDate).toLocaleDateString()}</p>
                                                </div>
                                            </div>
                                            <p className="text-sm font-black text-orange-500">{(order.totalAmount || 0).toLocaleString()}đ</p>
                                        </label>
                                    ))
                                )}
                            </div>
            ...
            <style dangerouslySetInnerHTML={{ __html: `.custom-scrollbar::-webkit-scrollbar { width: 6px; } .custom-scrollbar::-webkit-scrollbar-track { background: transparent; } .custom-scrollbar::-webkit-scrollbar-thumb { background: ${darkMode ? '#4b5563' : '#cbd5e1'}; border-radius: 10px; }` }} />
        </div>
    );
};
