...
export default function CheckoutTabScreen() {
    const insets = useSafeAreaInsets();
    const router = useRouter();
    const params = useLocalSearchParams<{ selectedIds?: string | string[] }>();

    const rawSelectedIds = Array.isArray(params.selectedIds)
        ? params.selectedIds.join(",")
        : params.selectedIds ?? "";

    const selectedIdSet = useMemo(
        () => new Set(rawSelectedIds.split(",").filter(Boolean)),
        [rawSelectedIds],
    );
    ...
    const selectedItems = useMemo(
        () => items.filter((item) => selectedIdSet.has(item.productId) && item.quantity > 0),
        [items, selectedIdSet],
    );
    ...
    const vatAmount = Math.round(selectedSubtotal * taxRate);
    const totalAmount = selectedSubtotal + vatAmount + shippingCostBase;
    ...
    const handleSubmitOrder = async () => {
        ...
        if (selectedItems.length === 0) {
            showToast("Không có sản phẩm hợp lệ để thanh toán.", "error");
            setSubmitError("Danh sách sản phẩm thanh toán đang trống.");
            return;
        }
        ...
        try {
            ...
            // Remove only purchased lines from cart after order is created.
        ...
        <ScrollView contentContainerStyle={[styles.content, { paddingTop: 16 + insets.top }]}>
            ...
            {selectedItems.length === 0 ? (
                <View style={styles.emptyWrap}>
                    <Text style={styles.emptyText}>Không có sản phẩm được chọn.</Text>
                    <Pressable style={styles.backToCartBtn} onPress={() => router.replace("/(tabs)/cart" as any)}>
                        <Text style={styles.backToCartText}>Về giỏ hàng</Text>
                    </Pressable>
                </View>
            ) : null}
